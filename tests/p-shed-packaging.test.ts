// Packaging guard for p-shed, mirroring tests/p-tasks-packaging.test.ts (regression:
// p-tasks 1.1.0 shipped with no js-yaml, so the CLI died with ERR_MODULE_NOT_FOUND
// once copied into the plugin cache).
//
...
import { cpSync, mkdtempSync, readFileSync, rmSync, statSync, readdirSync } from 'node:fs';
...
const PSHED_DIR = join(repoRoot(), 'plugins', 'p-shed');
const VENDORED_YAML = join(PSHED_DIR, 'tools', 'lib', 'vendor', 'js-yaml.mjs');
...
describe('p-shed packaging — self-contained at rest', () => {
  it('no tool source imports a bare runtime dependency', () => {
    const offenders: string[] = [];
    for (const file of toolSources(join(PSHED_DIR, 'tools'))) {
  ...
  it('ships the vendored js-yaml build and imports it relatively', () => {
    ...
    const ioMjs = readFileSync(join(PSHED_DIR, 'tools', 'lib', 'io.mjs'), 'utf-8');
    expect(ioMjs).toMatch(/from\s+['"]\.\/vendor\/js-yaml\.mjs['"]/);
  });
});
...
describe('p-shed packaging — CLI runs from an isolated copy', () => {
  let pkgRoot: string; // isolated copy of plugins/p-shed (no node_modules above it)
  let cli: string;
  let dataDir: string; // throwaway "repo" the CLI writes into (not a git repo —
  // p-shed's findRoot() falls back to cwd when it can't find a .git, so .pshed/
  ...
  beforeAll(() => {
    ...
    pkgRoot = mkdtempSync(join(tmpdir(), 'pshed-pkg-'));
    cpSync(PSHED_DIR, pkgRoot, { recursive: true });
    rmSync(join(pkgRoot, 'tools', '__tests__'), { recursive: true, force: true });
    cli = join(pkgRoot, 'tools', 'pshed.mjs');
    dataDir = mkdtempSync(join(tmpdir(), 'pshed-data-'));
  });
  afterAll(() => {
  ...
  // Note: unlike p-tasks, p-shed's tool does not read CLAUDE_PLUGIN_ROOT — its
  // root comes from findRoot(cwd) walking up to a .git dir, falling back to cwd.
  // So we run with cwd = dataDir and pass no special env.
  ...
  it('runs --version, set-job, and tick with valid JSON and no ERR_MODULE_NOT_FOUND', () => {
    let r = run(['--version'], dataDir);
    expect(r.stderr).not.toContain('ERR_MODULE_NOT_FOUND');
    expect(r.status).toBe(0);
    expect(r.stdout.trim()).toMatch(/^\d+\.\d+\.\d+$/);

    // Writes .pshed/jobs.yml via the vendored yaml.dump().
    r = run(
      ['set-job', '--schedule', '*/15 * * * *', '--prompt', 'do it', '--id', 'smoke', '--json'],
      dataDir,
    );
    expect(r.stderr).not.toContain('ERR_MODULE_NOT_FOUND');
    expect(r.status).toBe(0);
    expect(JSON.parse(r.stdout)).toMatchObject({ id: 'smoke', created: true });

    // Reads .pshed/jobs.yml via the vendored yaml.load().
    r = run(['tick', '--json'], dataDir);
    expect(r.stderr).not.toContain('ERR_MODULE_NOT_FOUND');
    expect(r.status).toBe(0);
