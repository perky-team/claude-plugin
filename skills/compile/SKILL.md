...
For each group, list each page as `- [<title>](pages/<type>/<slug>.md) — <one-line summary from frontmatter or first body sentence>`. If a group has no pages, render `_(none yet)_` instead of an empty list. Overwrite `<root>/docs/wiki/index.md`.
