...
`model` (passed to `claude --model`), and optional `maxConsecutiveFailures` (circuit
breaker threshold; default 3, `0` disables). To modify an existing job, pass its
`--id`. Run:
    node "${CLAUDE_PLUGIN_ROOT}/tools/pshed.mjs" set-job --schedule "<cron>" --prompt "<text>" [--id <id>] [--cwd <path>] [--timeoutSec <n>] [--permission-mode <mode>] [--allowed-tools "<list>"] [--model <name>] [--max-consecutive-failures <n>] --json
