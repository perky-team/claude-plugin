...
**Canonical mode (p-tasks present):** the `## Steps` section is **not** written to plan.md — the step list lives in p-tasks as sub-tasks. plan.md keeps only `## Overview`, `## Risks`, `## Open questions`, and (after reviews) `## Review decisions (audit)`. The table above describes the legacy (p-tasks-absent) plan.md; the canonical slim plan.md comes from `_shared/templates/plan-tasks.template.md`. Both paths are gated by `_shared/ptasks-bridge.md`.
...
| `tests/p-flow-ptasks-bridge.test.ts` | p-tasks bridge stays decoupled (no `plugin.json#dependencies`, no `ptasks.mjs` in any skill) and gated (host skills reference `_shared/ptasks-bridge.md`; bridge doc keeps "absent → silent no-op" AND the canonical-store rule: p-tasks owns the steps, plan.md keeps only narrative, slim template has no `## Steps`) |
