...
| Two execution modes: `executing-plan` (INLINE — implement in the current session) and `subagent-driven-development` (ISOLATED — fresh implementer subagent per step + per-step review, artifacts handed over as files under `.p-flow/sdd/`). `writing-plan` hands off via a numbered menu that recommends `subagent-driven-development` as the default (it keeps the crowded post-brainstorm context clean); `executing-plan` is the inline opt-in. SDD dispatches `Task` + `general-purpose` + colocated inline templates (never registered subagents), reuses `requesting-code-review/code-reviewer.md` for the final broad review, and uses the same p-tasks/checkbox ledger via the p-tasks gate. No external-plugin coupling (no `superpowers`, no `.superpowers/`). | H | `docs/plans/2026-07-01-subagent-driven-development.md`, `skills/subagent-driven-development/SKILL.md` |
| SDD workspace (`.p-flow/sdd/`) cleanup lives in **`task-end`, after a successful push** — NOT in `subagent-driven-development`. The artifacts double as SDD's resume journal (its resume path reads `.p-flow/sdd/task-<n>-*` to reconcile an interrupted `in_progress` step), so clearing them mid-run would destroy recovery state. A successful push is the only point where the run is finished and nothing is left to resume. Cleanup keeps `.p-flow/sdd/.gitignore` (holds `*`), is a no-op when the folder is absent (inline `executing-plan` was used), skips on a failed/absent push, and never touches `.claude/.p-flow-state/` (the verification marker). | H | `skills/task-end/SKILL.md` `## SDD workspace cleanup`, `skills/subagent-driven-development/SKILL.md` `## Workspace` |
...
| Non-interactive mode is an **env-var gate in a shared doc**, not a per-skill flag or a new skill: `P_FLOW_NONINTERACTIVE=1` (exactly `1`; anything else → interactive, silent no-op, byte-for-byte unchanged — same discipline as the p-tasks bridge). The doc supplies the **gate + the rule** (never ask; apply a default documented next to the question it replaces, or fail loudly with a named reason; never invent an answer that is the user's to decide); each host skill supplies its own **defaults**. First host: `requesting-code-review` (precondition 3 + §4 triage). Headless **requires canonical mode** — p-tasks absent, or a `jira` destination whose writes need a human yes, stops with a named reason rather than falling back to legacy or guessing. **The headless parent task is NEVER resolved from the branch** (an autonomous loop sits on one long-lived branch and takes a different task each run): it comes from the skill's `--task <t-id|task-title>` argument, else from the single `in_progress` task when there is exactly one, else a stop that names which input was missing. `reject` is unavailable headless; nothing is dropped, so the audit trail keeps the same shape in both modes. `code-reviewer.md` stays mode-neutral (SDD reuses it). | K | `skills/_shared/noninteractive.md`, `docs/specs/2026-08-07-noninteractive-mode.md` |
...
| `tests/p-flow-noninteractive.test.ts` | non-interactive mode stays a shared gate (`_shared/noninteractive.md` exists, pins `P_FLOW_NONINTERACTIVE` + the strict `= "1"` test, "anything else → silent no-op / byte-for-byte unchanged", and the never-invent-an-answer rule); `requesting-code-review` references it; the **interactive** three-option Blocker question and the Suggestions batch question survive verbatim; the **non-interactive** triage block contains no question at all and makes `reject` unavailable; the **non-interactive** precondition resolves the parent task from the `--task <t-id|task-title>` argument (documented, exact form) or a single `in_progress` task, never from the branch; headless failure paths are named and distinct (p-tasks absent, Jira writes, no-argument-and-none, no-argument-and-N, argument-matches-nothing); `allowed-tools` covers the new path; `code-reviewer.md` stays mode-neutral |
...
├── skills/
│   ├── _shared/templates/       ← templates (5 init-installed + 2 plan-internal = 7 files)
...
│   ├── _shared/noninteractive.md ← headless-run gate (P_FLOW_NONINTERACTIVE=1) + the never-ask rule
