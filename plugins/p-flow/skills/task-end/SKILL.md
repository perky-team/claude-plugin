...
3. **Plan completeness (soft warning).** Resolve `<slug>`:
   ...
   - **Canonical mode (p-tasks present):** there is **no `plan.md`** — do not read one. Via the Skill tool, `p-tasks:list <parent>` where the parent is the task titled `<slug>` (same object), and count the sub-tasks whose `status` ≠ `done` (these are the open plan steps **and** open review-origin follow-ups — both live as sub-tasks).
