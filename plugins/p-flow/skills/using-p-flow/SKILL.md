...
| `subagent-driven-development` | **Recommended default** after the plan is approved — `writing-plan` hands off to it unless the user asks for inline. Gives **per-step context isolation**: a fresh implementer subagent per step + a per-step review (spec + quality), broad review at the end. Keeps the crowded post-brainstorm main context clean; same p-tasks/checkbox ledger as `executing-plan`. |
| `executing-plan` | The inline alternative — use when the user explicitly opts to implement **inline in this session** instead of the default SDD mode. Drives the plan steps in order — TDD for code steps, verify after each, mark done only on green. Steps live in plan.md `## Steps` (legacy) or as p-tasks sub-tasks (canonical). |
...
- **Canonical mode uses a three-state status lifecycle.** A sub-task is `todo` (not started) → `in_progress` (set the moment work begins — before implementing, or before dispatching the implementer subagent) → `done` (only when its acceptance criterion is verified green). The parent goes `in_progress` on the first step. On resume, an `in_progress` sub-task means the run was **interrupted** mid-step — reconcile it (check git + artifacts, verify, then finish or redo) rather than assuming done or blindly re-running. Legacy plan.md checkboxes are binary and can't express in-progress. See `skills/_shared/ptasks-bridge.md` § Status lifecycle.
