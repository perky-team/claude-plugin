...
| `executing-plan` | After the plan is approved and you're about to implement. Drives the plan steps in order — TDD for code steps, verify after each, mark done only on green. Steps live in plan.md `## Steps` (legacy) or as p-tasks sub-tasks (canonical). The loop between `writing-plan` and `task-end`. |
...
- **p-tasks is optional but canonical when present.** If (and only if) p-tasks is initialised in the repo (`docs/tasks/.ptasks.json` exists), it is the **single source of truth** for the step list and statuses: `writing-plan` creates one sub-task per step (no `## Steps` in plan.md), `executing-plan` walks them, the review skills add follow-ups as sub-tasks, and `task-end` counts/closes them. plan.md then holds only `## Risks` / `## Open questions` / `## Review decisions (audit)`. When p-tasks is **absent**, behaviour is byte-for-byte the legacy plan.md-only flow (`## Steps` checklist). Creating real Jira issues still requires explicit confirmation.
