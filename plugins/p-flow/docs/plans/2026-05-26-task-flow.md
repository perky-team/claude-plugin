...
## Post-implementation amendments (2026-05-27)

After all 11 implementation tasks landed, a final code review of the cumulative branch diff surfaced design defects that originated in this plan, not in transcription. The following amendments were applied to the affected SKILL.md / agent files on the same branch. The original task text above is preserved as historical record; the current truth lives in the files.

### Task 9 (`task-start`)

- **Critical fix to Phase B step 7:** the original "instead run `git switch <branch>`" wording was ambiguous and would, on a literal reading, run `git branch <branch>` against an existing branch and fail with `fatal: A branch named '<branch>' already exists`. Rewritten as two explicit case branches.
- **`--worktree` semantics clarified:** the skill now stops after creating the worktree (printing a "start a new Claude Code session there" message) and does NOT proceed to `mkdir specs/<slug>/` or invoke `task-brainstorming` in the *original* checkout. The new worktree session picks up from step 9. This was the only viable contract given that the current session's CWD cannot follow `git worktree add`.
- **`allowed-tools` minimised:** removed `Bash(git checkout:*)` (skill uses `git switch` throughout).

### Task 10 (`task-end`)

- **Slug-resolution fallback added (pre-check 3):** if the current branch does not match `<type>/<slug>` for `<type>` in {`feature`, `bugfix`, `hotfix`, `chore`, `docs`}, the skill now asks the user for `<slug>`; if the user can't supply one, pre-checks 3 and 4 are skipped with a one-line warning, and the skill proceeds to push.
- **Default branch fallback (step 7):** try `main` → `master` → `git remote show origin | grep 'HEAD branch'` → ask the user. Same fallback added to `requesting-code-review` and `requesting-task-review`.
- **MR title made deterministic (step 7):** single non-merge commit → use subject verbatim. Multiple → squash subject `<dominant-type>(<slug>): <one-line summary from spec Overview>`, with a precedence list for ties.
- **Step 9 placeholders renamed:** `<title>` / `<body>` / `<body-escaped>` → `{{TITLE}}` / `{{BODY}}` / `{{BRANCH}}`, with an explicit instruction to substitute (not print literally). Both `gh` and `glab` commands now use the same heredoc-escaped body.
- **`allowed-tools` extended:** added `Bash(git merge-base:*)`, `Bash(git worktree:*)`, `Bash(git remote:*)` to match what the body actually uses.

### Task 2 (`verification-before-completion`)

- **No-tests path tightened:** the "No tests found" and "Tests fail" bullets in step 5 now explicitly state "Do NOT write the success marker" and "Stop". The Hard rules were tightened accordingly: "Marker is written only when at least one verification command actually ran and returned exit code 0."

### Task 7 (`requesting-code-review`)

- **Base-branch resolution added** as precondition 1 (see Task 10 amendment above).
- **"three buttons" wording** in §4 triage protocol changed to **"three options"** for platform neutrality.

### Task 8 (`requesting-task-review`)

- **Base-branch resolution added** as precondition 1.
- **Fourth severity `note` removed.** The triage protocol now uses the same three-severity model as `requesting-code-review` (blocker / suggestion / nit). Scope creep is mapped to **suggestion** so it lands in the audit log on `defer`/`reject` and can be converted to a follow-up on `fix`.

### Task 6 (`task-reviewer` agent)

- **Summary section aligned** to the three-severity model (Blockers / Suggestions / **Nits**, plural). Scope creep is folded into Suggestions; the "Notes" tier is gone.

### Task 4 (`task-brainstorming`)

- **Precheck symmetry:** the "no placeholders" case used to silently switch to refinement mode. It now shows the same menu the "placeholders present" case shows — *refine in place / discard and restart / cancel* — so a user who invoked `task-start` intending a fresh start can recover.

### Not addressed (deferred)

The final review also produced 8 Minor findings (severity heading sing/plural in agent output examples, placeholder format in the `writing-plan` template, README's "auto-invoked" wording, etc.). These are deferred to a follow-up wave; none affect runtime behaviour of the surface.
