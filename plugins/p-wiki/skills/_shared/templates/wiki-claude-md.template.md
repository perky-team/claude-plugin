...
- **Ranked search** — `pwiki search "<question>" --format=json --limit=10` (unions the primary with every entry in the config's `sources` array, including `gitlab`/`github`/`http` sources; each result carries a `source` field and the JSON includes a `warnings` array for unreachable sources).
- **Reading a page** — `pwiki get <path> [--format=json]` (returns the page's frontmatter + body for FS *or* Confluence; use this instead of the `Read` tool for any wiki page, since `Read` only opens local files). Add `--source=<name>` to read from a configured read-only source — works for `gitlab`/`github`/`http` sources as well as `fs` and Confluence (omit for the primary).
...
- **Index regeneration** — `pwiki index` (default `--format=json`): rewrites `docs/wiki/index.md` from frontmatter *and* writes `docs/wiki/index.json`, the read-only bundle consumed by `gitlab`/`github`/`http` sources in other wikis. `--format=text` prints `index.md` to stdout without writing either file.
- **Bundle rebuild** — `pwiki reindex`: same effect as `pwiki index` — regenerates both `docs/wiki/index.md` and `docs/wiki/index.json`. Run either before `git push` whenever you want consumers to see the latest pages.
- **Connecting another wiki as a read-only source** — `pwiki source add <name> --kind=fs --path=<that repo root>` for a local clone, `--from-config=<path to that wiki's .pwiki.json> [--from-destination=<block>]` to copy a whole block (the only practical way to add a Confluence source), or `--kind=github --owner=… --repo=…` / `--kind=gitlab --project=…` / `--kind=http --url=…` to read a published `index.json` without cloning. The command validates the config and probes the source, writing nothing if the probe fails (`source-unreachable`); `--no-verify` skips the probe. Never hand-edit `sources` / `destinations` in `.pwiki.json` — use this command.
...
- Pages live under the configured `rootPageId`, organized by sub-parents (Concepts, People, Sources, Queries) plus an Index page regenerated by `pwiki index`. These structural pages are titled `<titlePrefix> — <Concepts|People|…|Index>` so multiple wikis can coexist in one space (Confluence requires space-wide unique titles). `titlePrefix` defaults to the root page title at init and is stored in `.pwiki.json`; content page titles are never prefixed, and structural pages are still discovered by their `pwiki-role` property, not by title.
...
To populate the mirror, run `/p-wiki:sync` (or `pwiki sync` from the shell). It walks the primary, writes every page into each mirror (with cross-link `href`s translated to the mirror's format), deletes mirror-only pages (true-mirror semantics), and regenerates the Index on each mirror.
