...
describe('js extraction (javascript grammar — no TS-only node types)', () => {
  it('compiles the js query and captures class, methods, function, calls, import', async () => {
    const cfg = resolveLang('svc.mjs'); // langId 'javascript'
    const { nodes, edges } = await extract({ file: 'svc.mjs', lang: cfg.lang, langId: cfg.langId, scm: cfg.query, source: SRC });
