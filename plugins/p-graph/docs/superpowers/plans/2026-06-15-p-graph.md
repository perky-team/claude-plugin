...
- [ ] **Step 3: Implement the engine wrapper.** Uses the **verified** `web-tree-sitter@0.25.10` API (confirmed by the Task 0.3 probe): the vendored `web-tree-sitter.js` is an **ESM module with named exports** `{ Parser, Language, Query }` — load it via dynamic `import()` of a file URL. `Parser.init({locateFile})` → `Language.load(path)` → `new Parser(); setLanguage; parse(string)` → `new Query(lang, scm); query.matches(root)` with `match.captures[].name` and `.node`.
...
> Verified working namespace: `{ CaptureQuantifier, LANGUAGE_VERSION, Language, LookaheadIterator, MIN_COMPATIBLE_VERSION, Node, Parser, Query, Tree, TreeCursor }`. Grammars live directly in `tools/lib/grammars/<langId>.wasm` (no nested `grammars/grammars`).
